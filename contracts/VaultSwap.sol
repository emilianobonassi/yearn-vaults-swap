...
contract VaultSwap {
    ...
    modifier onlyCompatibleVaults(address vaultA, address vaultB) {
        require(IVaultAPI(vaultA).token() == IVaultAPI(vaultB).token(), "Vaults must have the same token");
        _;
    }
    ...
    function _permit(address vault, uint256 value, uint256 deadline, bytes calldata signature) internal {
        require(IVaultAPI(vault).permit(
            msg.sender,
            address(this), 
            value,
            deadline,
    ...
    function _swap(
        ...
        uint256 balanceVaultFrom = vf.balanceOf(address(this)).sub(preBalanceVaultFrom);


        // Withdraw token from faultFrom
        ...
        uint256 balanceToken = token.balanceOf(address(this)).sub(preBalanceToken);
        ...
        if(balanceVaultFrom > 0) {
            vf.safeTransfer(msg.sender, balanceVaultFrom);
        }
        ...
        if(balanceToken > 0) {
            token.safeTransfer(msg.sender, balanceToken);
        }
    }
